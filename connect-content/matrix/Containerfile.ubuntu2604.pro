...
COPY connect-content/matrix/deps/ubuntu-26.04_packages.txt /tmp/ubuntu-26.04_packages.txt
...
RUN /opt/R/$R_VERSION/bin/R -e 'install.packages("odbc", repos="https://p3m.dev/cran/__linux__/resolute/latest")'
...
RUN GH_TOKEN="$([ -s /run/secrets/github_token ] && cat /run/secrets/github_token)" /opt/quarto/bin/quarto install tinytex --no-prompt --update-path && \
